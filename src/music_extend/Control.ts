...
export default class Control {
    ...
    private fullDestructionTab(): [string, string, string, string] {
        ...
        const [note, orginalLength] = this.tab.slice(this.currentElement!.startChar, this.currentElement!.endChar).split('/');
        return [part1, part2, note, orginalLength]
    }
    private partialDestructionTab(): [string, string, string] {
        const part1 = this.tab.substring(0, this.currentElement!.startChar);
        const part2 = this.tab.substring(this.currentElement!.endChar);
        const note = this.tab.slice(this.currentElement!.startChar, this.currentElement!.endChar);
        return [part1, part2, note]
    }
    public changeNote(char: string) {
        if (this.currentElement === null) return;
        const [part1, part2, _, orginalLength] = this.fullDestructionTab();
        const length = orginalLength ? '/' + orginalLength : '';
        // setCurrentElement(null);
        this.setTab(part1 + char + length + part2)
    }
    ...
    private getNextNonBarElement() {
        let nextElement = this.tune!.getElementFromChar(this.currentElement!.endChar) as abcjs.AbcElem;
        if (!nextElement) return;
        if (nextElement.el_type === 'bar') {
            nextElement = this.tune!.getElementFromChar(nextElement.endChar) as abcjs.AbcElem;
        }
        return nextElement;
    }
    public removeTie() {
        ...
        const betweenNote1Note2 = this.tab.slice(this.currentElement.endChar, note2Element.startChar)
        // get text of note 2
