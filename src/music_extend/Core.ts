...
class CursorControl {
  onReady() {
    let clickEl = document.querySelector(
      ".click-explanation"
...
interface SettingTune {
  ...
  setTuneObj: React.Dispatch<
    React.SetStateAction<abcjs.TuneObject | undefined>
  >;
  index?: number;
}
...
export function load(settingTune: SettingTune) {
  if (abcjs.synth.supportsAudio()) {
    ...
    (document.querySelector("#audio") as HTMLDivElement).innerHTML =
...
function setTune(
  ...
  const clickListener: abcjs.ClickListener = (
    abcElem,
    tuneNumber,
    classes,
    analysis,
      ...
      afterParsing: (tune, tuneNumber, abcString) => {
        if (currentElement) {
          setTimeout(() => {
            ...
            if (action === "ADD") {
              newElement = tune.getElementFromChar(
  ...
  const midi = abcjs.synth.getMidiFile(tab, {
    fileName: "i-was-not-insane.midi",
  });
