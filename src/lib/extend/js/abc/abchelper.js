...
import {
    ...
    SEVERITY_RED,
    SEVERITY_RED_COLOR,
    SEVERITY_YELLOW_COLOR,
} from "../analysis/AnalysisResults.js";
...
export function highlightNote() {
    ...
    if (!vc) {
        console.log(
            json_stringify_circular(nd),
            json_stringify_circular(selected)
        );
        ...
        for (let voice = 0; voice < voices.length; voice++) {
            ...
            for (let elem = 0; elem < elems.length; elem++) {
                ...
                if (elStart in nd.abc_charStarts) {
                    ...
                    nd.voices[el.voice].notes[el.note].abcelem =
                        elems[elem].abcelem;
                }
            }
        }
    ...
    for (let v = 0; v < nd.voices.length; ++v) {
        ...
        for (let n = 0; n < vc.notes.length; ++n) {
            ...
            if (nt.d && settings.show_nht && nd.algo === "CA3") {
                ...
                if (msh < 0)
                    draw_circle_around_notehead(nt.abcelem.abselem, svg);
            }
        }
    }
}
...
function draw_glis(v, n1, n2, sev, svg) {
    ...
    if (abselem1.counters.line === abselem2.counters.line) {
        const len = Math.sqrt(
            (np2.y - np1.y) * (np2.y - np1.y) +
                (np2.x - np1.x) * (np2.x - np1.x)
        );
...
function notation_redraw() {
    try {
        ...
        console.log(e);
        if (
            e == "Can't find variable: ABCJS" ||
            e == "ReferenceError: ABCJS is not defined"
        ) {
            trackEvent("AiHarmony", "error_not_found_abcjs");
            alertify.warning(
                `Please check your internet connection and reload page. Could not load music visualization module.`,
                60
            );
        } else {
            throw e;
