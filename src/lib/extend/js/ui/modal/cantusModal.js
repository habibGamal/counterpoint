...
// import ABCJS from '../../../plugin/abcjs-6.0.0/abcjs_basic_6.0.0-beta.26.js'
...
function bestTranspose(minD, maxD, rangeMinD, rangeMaxD, step) {
    ...
    while (1) {
        ...
        if (range_penalty2 >= range_penalty || maxD + transpose > 127 || minD + transpose < 0) {
...
function cantusToData(cid, arrangement, keysig, timesig) {
    let [d, alter] = transposeCantus(cid, arrangement, keysig);
    nd.set_name(
        `Cantus ${cid + 1} in ${arrangement.cantus}, ${arrangement.parts.length} parts, ${keysig.name}, ${
            timesig.beats_per_measure
        }/${timesig.beat_type}`
    );
...
function updateCantusPreview(cid) {
    const timesig = timesigs.find(
        (timesig) => timesig.beats_per_measure + "/" + timesig.beat_type === $("#selectTimeSig option:selected").val()
    );
    ...
    if (arrangement.parts.length < 2) {
        ...
        document.getElementById(
            "cantusAbc"
        ).innerHTML = `<span style='color:red'>Set at least one part to Counterpoint</span>`;
    } else if (arrangement.cantus === "") {
        $("#modalOk").prop("disabled", true);
        document.getElementById(
            "cantusAbc"
        ).innerHTML = `<span style='color:red'>Please set one part to Cantus firmus</span>`;
    } else {
...
export function showCantusModal2(cid) {
    ...
    st += `<tr style="display:none"><td colspan=2><hr>`;
    st += `<tr style="display:none"><td>`;
    st += `<b>Key:</b><td>`;
    ...
    $("#modalOk").click(() => {
        ...
        console.log(
            ...
            timesigs.find(
                (timesig) =>
                    timesig.beats_per_measure + "/" + timesig.beat_type === $("#selectTimeSig option:selected").val()
            )
        );
        cantusToData(
            cid,
