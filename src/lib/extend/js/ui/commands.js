...
export function toolbarButtonHtml(command, showHints) {
  ...
  if (command.toolbar.type !== 'select') {
    ...
    st += `<a ${tooltip} id='${command.id}' class='btn btn-primary btn-outline-white px-4 p-1 ${command.toolbar.disabled ? "disabled " : ""}' href=# role='button' style='display: flex; justify-content: center; font-weight:bold; flex-shrink: 0; align-items: center; min-width: ${mobileOpt[mobileOrTablet].toolbar_button_width}px; font-size: ${command.toolbar.fontSize * mobileOpt[mobileOrTablet].toolbar_font_scale || '1'}em'>`;
  }
...
function makeToolbar(toolbar_id) {
  ...
  for (let command of commands) {
    ...
    if (command.separator) {
      st += `<div class="rounded-xl w-[4px] h-[30px] bg-white opacity-40"></div>${nbsp}`;
      continue;
    }
...
export function initCommands() {
  ...
  document.getElementById("toolbar4").innerHTML = makeToolbar(4);
  for (let command of commands) {
...
export let commands = [
  ...
  // {
  //   id: 'cantus',
  //   toolbar: {type: 'image', toolbar_id: 2, hintText: 'Cantus'},
  //   event: 'onclick',
  //   keys: [],
