...
import alertify from "../../plugin/alertifyjs/alertify.js";
let workers = {};
...
async function workerMessageReceiver(event) {
    ...
    if (type === "ERROR") {
        ...
        if (typeof data.startsWith === "function" && data.startsWith("Timeout ")) {
            ...
            alertify.warning(
                `Please check your internet connection and reload page. Error loading music analysis module.`,
                60
            );
        } else {
            alertify.warning(
                `If analysis is not working, try to <a href='https://www.digitaltrends.com/computing/how-to-clear-your-browser-cache/' target=_blank>clear Cached Images and Files</a>, reopen browser and reload page`,
                60
            );
            throw data;
        }
