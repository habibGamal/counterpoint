...
const IllustrationWithSound = ({ src, desc, audio }: { src: string; desc: string; audio: HTMLAudioElement }) => {
    const toggle = () => {
        if (audio.paused) {
            audio.play();
        } else {
    ...
    return (
        <div className="relative rounded-xl my-4 text-xl shadow-lg bg-[#DBF8FD] border border-sky-900 text-sky-900 font-medium overflow-hidden">
            ...
            <p className="p-4 flex items-center gap-4">
                ...
                >
                    {audio.paused ? <PlayCircle size="32" color="#fff" /> : <PauseCircle size="32" color="#fff" />}
                </div>
                {desc}
            </p>
        </div>
